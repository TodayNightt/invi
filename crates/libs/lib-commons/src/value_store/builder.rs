...
impl Builder {
    ...
    pub fn string(mut self, key: impl AsRef<str>, value: impl AsRef<str>) -> Self {
        self.values
            ...
            .or_insert(Value::String(value.as_ref().to_string()));
        self
    }
    ...
    pub fn array(
        mut self,
        key: impl AsRef<str>,
        value: impl Into<Vec<Value>>,
        schema: Option<&str>,
    ) -> Self {
        self.values
            .entry(key.as_ref().to_string())
            .or_insert(Value::Array(value.into()));

        if let Some(schema) = schema {
            self.object_properties_schemas
                .entry(key.as_ref().to_string())
                .or_insert(schema.into());
        }

        self
    }

    pub fn object<K, V, T>(mut self, key: impl AsRef<str>, value: T, schema: Option<&str>) -> Self
    where
        K: Into<String>,
        V: Into<Value>,
    ...
    pub fn with_value(mut self, value: &Value) -> Result<Self> {
        self.values = value
            .as_object()
            .ok_or(ValueStoreError::NotAnObject)?
            .to_owned();
        Ok(self)
    }
